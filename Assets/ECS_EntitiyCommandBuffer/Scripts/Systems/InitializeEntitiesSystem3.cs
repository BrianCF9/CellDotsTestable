...
{
    public class InitializeEntitiesSystem3 : SystemBase
    {
        ...
        protected override void OnUpdate()
        {
            ...
            Entities.WithAll<ShouldSpawnTag3>().ForEach((Entity _, int entityInQueryIndex, ref EntitySpawnData3 spawnData, in Translation translation) =>
            {
                if (!spawnData.isInitialized)
                {
                    ...
                    ecb.AddComponent<CellTag3>(entityInQueryIndex, entity);
                    ecb.AddComponent<NonUniformScale>(entityInQueryIndex, entity);
                    ecb.SetComponent(entityInQueryIndex, entity, new Translation
                    {
                        Value = new float3(0,0.5f,1f)
                    });
                    ecb.SetComponent(entityInQueryIndex, entity, new CellTag3
                    {
