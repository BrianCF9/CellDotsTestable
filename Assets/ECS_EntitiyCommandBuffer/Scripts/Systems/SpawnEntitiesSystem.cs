...
namespace TMG.ECS_CommandBuffer
{
    public class SpawnEntitiesSystem : SystemBase
    {
        ...
        protected override void OnUpdate()
        {
            ...
            Entities.ForEach((Entity _, int entityInQueryIndex, ref CellTag cellTag, ref NonUniformScale scale, in Translation translation) =>
            {
                ...
                if (cellTag.readyToSpawn && cellTag.actualTimeToGrowth >= cellTag.timeToGrowth)
                {
                        ...
                        ecb.AddComponent<CellTag>(entityInQueryIndex, newEntity);
                        if (xyz == 0)
                        {
                            if (upOrDown == 0)
                        {
                        ...
                        ecb.SetComponent(entityInQueryIndex, newEntity, new CellTag
                        {
